...
const ParentListPage = async ({
    ...
    return (
      ...
      >
        ...
        <td className="hidden md:table-cell">
          {item.students.map((student) => student.name).join(", ") || "N/A"}
        </td>
        ...
        <td>
          <div className="flex items-center gap-2">
            <Link href={`/parents/${item.id}`}>
              ...
              <span className="w-7 h-7 text-white flex items-center justify-center rounded-full bg-lamaSky">
                <Eye size={16} /> {/* Eye icon for view/edit */}
              </span>
            </Link>
            {role === "admin" && (
  ...
  try {
    ...
    return <div>Error loading data.</div>; // Or a more user-friendly error message
  }
};
